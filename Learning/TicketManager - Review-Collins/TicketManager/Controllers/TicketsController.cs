...
namespace TicketManager.Controllers
{
    ...
    public class TicketsController : ControllerBase
    {
        ...
        {
            var tickets = await _ticketRepository.GetFiltered(t => t.assignee.ToLower().Contains(assignee.ToLower()));
        ...
        [HttpPatch("{id}/attachment_upload")]
        public async Task<IActionResult> AddAttachmentToTicket(int id, IFormFile file)
        {
            var existingAttchments = await _ticketRepository.GetFilesNames(id);
            if (existingAttchments.Find(x => x.Contains(Path.GetFileNameWithoutExtension(file.FileName)) && x.Contains(Path.GetExtension(file.FileName))) != null)
                return BadRequest("File Already exists!");

            await _ticketRepository.UploadFileToTicket(id, await _attachments.Upload(file));

            return Ok("File added to ticket successfully");
        }
        ...
        [HttpDelete("{id}/{fileName}/delete")]
        public async Task<IActionResult> DeleteFileFromTicket(int id, string fileName)
        {
            if (!ModelState.IsValid)
                return BadRequest();
            ...
            if (!ticket.attachments.Contains(fileName))
                return NotFound($"\"{fileName}\" doesn't exist in the current ticket");

            try
            {
                await _ticketRepository.RemoveFileFromTicket(ticket, id, fileName);
                return NoContent();
            }
            catch (Exception)
            {
